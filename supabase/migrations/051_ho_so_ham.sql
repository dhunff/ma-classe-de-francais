-- 051 — hàm ghi hồ sơ mở rộng của CHÍNH MÌNH, và quyền gọi
--
-- Cần 049 (cột) và nên chạy sau 050 (chép dữ liệu). Xem đầu 049 để biết vì
...
begin
  ...
  if v_dob_txt is not null then
    ...
    /* Ràng buộc `check` của 049 KHÔNG kiểm được điều này: Postgres đòi biểu
...
exception
  /* Một trong tám ràng buộc của 049 từ chối: điện thoại sai khuôn, trình độ
