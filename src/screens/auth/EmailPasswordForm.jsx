...
import { signInWithGoogle } from "../../shared/googleAuth.js";
...
/* Logo Google, bốn màu chính thức. Vẽ thẳng bằng SVG chứ không tải ảnh: một
   biểu tượng 1KB không đáng một vòng đi mạng, và nút sẽ không bao giờ hiện ra
   trong trạng thái thiếu logo. */
function GoogleMark() {
  return (
...
export default function EmailPasswordForm({ accounts = [], onLogin, mode = "login", onModeChange, autoFocus = false }) {
  ...
  /* Không setBusy(false) khi trót lọt: trình duyệt đang rời trang, bật nút
     sáng lại chỉ kịp nhấp một cái rồi mất. Chỉ tắt cờ ở các nhánh hỏng. */
  const googleLogin = async () => {
    setError("");
    setNotice("");
    setBusy(true);
    const { reason } = await signInWithGoogle();
    if (reason === "disabled") { setError(t("login.err_google_disabled")); setBusy(false); return; }
    if (reason === "network") { setError(t("login.err_network")); setBusy(false); return; }
    if (reason === "failed") { setError(t("auth.google_failed")); setBusy(false); }
  };
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-5">
      ...
      {/* Google chỉ hiện ở đăng nhập và đăng ký. Ở màn "quên mật khẩu" nó vô
          nghĩa: tài khoản Google không có mật khẩu ở hệ thống này để mà đặt
          lại. */}
      {!isReset && (
        <>
          <div aria-hidden className="flex items-center gap-3">
            <span className="h-px flex-1 bg-line" />
            <span className="text-xs font-semibold text-soft">{t("auth.or")}</span>
            <span className="h-px flex-1 bg-line" />
          </div>
          ...
          <button
            ...
            onClick={googleLogin}
            disabled={busy}
            className="flex w-full cursor-pointer items-center justify-center gap-3 rounded-xl border border-solid border-line bg-surface px-5 py-3 font-[inherit] text-sm font-semibold text-ink shadow-sm transition-colors hover:bg-surface2 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <GoogleMark />
